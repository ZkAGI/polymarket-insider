...
/**
 * Telegram Bot Command Handlers - INSIDER INTELLIGENCE VERSION
 *
 * Real whale tracking with actionable signals:
 * - Wallet win rates and history
 * - Signal strength (follow/fade)
...
function mapChatType(chatType: string): TelegramChatType {
  switch (chatType) {
    case "private": return TelegramChatType.PRIVATE;
    case "group": return TelegramChatType.GROUP;
    case "supergroup": return TelegramChatType.SUPERGROUP;
    case "channel": return TelegramChatType.CHANNEL;
    default: return TelegramChatType.PRIVATE;
  }
}
...
*Polymarket Insider Intelligence*

━━━━━━━━━━━━━━━━━━━━━━

📊 *Commands:*
/whales - 🐋 Whale trades with signals
/markets - 📈 Hot markets by volume
/insiders - 🕵️ Suspicious activity
/fresh - 🆕 New wallet big trades
/alerts - ⚙️ Configure notifications
/help - All commands

━━━━━━━━━━━━━━━━━━━━━━

*What makes us different:*
We don't just show trades - we show you
WHO is trading and whether to FOLLOW.

🟢 = Strong follow signal
🟡 = Mixed history  
🔴 = Consider fading

_Start with /whales to see smart money!_`;
}

// =============================================================================
// /whales - INTELLIGENT WHALE TRACKING
// =============================================================================
...
export async function getWhalesMessage(): Promise<string> {
  try {
    ...
    // Get whale trades with full wallet stats
    const trades = await prisma.trade.findMany({
      where: {
        createdAt: { gte: oneDayAgo },
        usdValue: { gte: 10000 },
    ...
    if (trades.length === 0) {
      return `🐋 *Whale Intelligence* (24h)

━━━━━━━━━━━━━━━━━━━━━━

No whale trades (>$10K) detected.

Markets are quiet - this could mean:
• Low volatility period
• Whales waiting for news
• Check back after major events

_Updated: ${new Date().toLocaleTimeString()}_`;
    }

    let message = `🐋 *Whale Intelligence* (24h)\n\n`;
    ...
    // Summary header
    const totalVol = totalVolume >= 1000000 
      ? `$${(totalVolume / 1000000).toFixed(1)}M` 
      : `$${(totalVolume / 1000).toFixed(0)}K`;
    message += `📊 *${trades.length} trades* | ${totalVol} volume\n`;
    message += `🟢 ${buyCount} buys | 🔴 ${sellCount} sells\n`;
    message += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;
    ...
    for (const trade of trades) {
      ...
      const severity = trade.usdValue >= 100000 ? "🚨" : trade.usdValue >= 50000 ? "⚠️" : "";
      
      const walletAddr = wallet?.address || trade.walletId;
      const walletShort = `${walletAddr.slice(0, 6)}...${walletAddr.slice(-4)}`;
      
      const marketQuestion = market?.question || "Unknown Market";
      const marketDisplay = marketQuestion.length > 40 ? marketQuestion.slice(0, 40) + "..." : marketQuestion;
      
      // Calculate signal strength
      const winRate = wallet?.winRate || 0;
      const tradeCount = wallet?.tradeCount || 0;
      const totalVolumeWallet = wallet?.totalVolume || 0;
      ...
      if (tradeCount >= 20) {
        ...
        signal = "🟡";
        signalText = "FEW TRADES";
      }
      
      // Check if fresh wallet (< 7 days old or < 5 trades)
      const isFresh = tradeCount < 5;
      const freshBadge = isFresh ? " 🆕" : "";
      
      // Whale badge
      const whaleBadge = wallet?.isWhale ? " 🐋" : "";
      
      // Format wallet volume
      const walletVol = totalVolumeWallet >= 1000000 
        ? `$${(totalVolumeWallet / 1000000).toFixed(1)}M`
        : totalVolumeWallet >= 1000 
          ? `$${(totalVolumeWallet / 1000).toFixed(0)}K`
          : `$${totalVolumeWallet.toFixed(0)}`;
      ...
      const polymarketUrl = eventSlug 
        ? `https://polymarket.com/event/${eventSlug}`
        : `https://polymarket.com/browse`;
      const polygonUrl = `https://polygonscan.com/address/${walletAddr}`;
      
      message += `${severity}${side} *${size}*${freshBadge}${whaleBadge}\n`;
      message += `📈 ${marketDisplay}\n`;
      message += `${signal} *${signalText}* | ${tradeCount} trades | ${winRate > 0 ? winRate.toFixed(0) + "% win" : "No history"}\n`;
      message += `👛 [${walletShort}](${polygonUrl}) | Vol: ${walletVol}\n`;
      message += `🔗 [View Market](${polymarketUrl})\n\n`;
    }

    message += `━━━━━━━━━━━━━━━━━━━━━━\n`;
    message += `_${new Date().toLocaleTimeString()} • /insiders for suspicious_`;
...
export async function handleWhalesCommand(ctx: Context): Promise<void> {
  await ctx.reply("🔄 Analyzing whale activity...");
  const message = await getWhalesMessage();
  await ctx.reply(message, { 
    parse_mode: "Markdown",
    link_preview_options: { is_disabled: true },
...
// =============================================================================
// /insiders - SUSPICIOUS ACTIVITY DETECTION
// =============================================================================

export async function getInsidersMessage(): Promise<string> {
  try {
    const { PrismaClient } = await import("@prisma/client");
    const prisma = new PrismaClient();
    ...
    // 1. New wallets with big trades
    // 2. Wallets trading only 1-2 markets heavily
    // 3. Unusually high win rates
    
    const suspiciousWallets = await prisma.wallet.findMany({
      where: {
        OR: [
          // Fresh wallets with significant volume
          { tradeCount: { lte: 10 }, totalVolume: { gte: 50000 } },
          // Very high win rate (possible insider)
          { winRate: { gte: 75 }, tradeCount: { gte: 10 } },
          // Flagged wallets
          { isFlagged: true },
      ...
      include: {
        trades: {
          ...
          include: {
            market: { select: { question: true, eventSlug: true } },
          },
        },
      },
    });
...
*What we look for:*
• New wallets with big trades
• Unusually high win rates
• Concentrated market focus
• Pre-event large positions

_Updated: ${new Date().toLocaleTimeString()}_`;
    }

    let message = `🕵️ *Insider Detection*\n\n`;
    message += `⚠️ *${suspiciousWallets.length} suspicious wallets*\n`;
    message += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;

    for (const wallet of suspiciousWallets) {
      const walletShort = `${wallet.address.slice(0, 6)}...${wallet.address.slice(-4)}`;
      
      const volume = wallet.totalVolume >= 1000000 
        ? `$${(wallet.totalVolume / 1000000).toFixed(1)}M`
        : `$${(wallet.totalVolume / 1000).toFixed(0)}K`;
      
      // Determine why suspicious
      const reasons: string[] = [];
      if (wallet.tradeCount <= 10 && wallet.totalVolume >= 50000) {
        reasons.push("🆕 New + Big volume");
      }
      if ((wallet.winRate || 0) >= 75 && wallet.tradeCount >= 10) {
        reasons.push(`🎯 ${wallet.winRate?.toFixed(0)}% win rate`);
      }
      if (wallet.isFlagged) {
        reasons.push("🚩 Flagged");
      }
      if ((wallet.suspicionScore || 0) >= 50) {
        reasons.push(`⚠️ Score: ${wallet.suspicionScore?.toFixed(0)}`);
      }
      
      const trades = (wallet as any).trades || [];
      const recentMarket = trades[0]?.market?.question || "Unknown";
      const marketDisplay = recentMarket.length > 30 ? recentMarket.slice(0, 30) + "..." : recentMarket;
      
      message += `👤 [${walletShort}](https://polygonscan.com/address/${wallet.address})\n`;
      message += `💰 ${volume} | ${wallet.tradeCount} trades\n`;
      message += `${reasons.join(" | ")}\n`;
      message += `📈 Latest: ${marketDisplay}\n\n`;
    }

    message += `━━━━━━━━━━━━━━━━━━━━━━\n`;
    message += `_DYOR - Not financial advice_`;
    
    return message;
  } catch (error) {
    console.error("[TG-BOT] Error fetching insiders:", error);
    return `🕵️ *Insider Detection*\n\n⚠️ Error analyzing activity. Try again later.`;
  }
}

export async function handleInsidersCommand(ctx: Context): Promise<void> {
  await ctx.reply("🔄 Scanning for suspicious activity...");
  const message = await getInsidersMessage();
  await ctx.reply(message, { 
    parse_mode: "Markdown",
    link_preview_options: { is_disabled: true },
  });
...
export function createInsidersCommandHandler(): (ctx: Context) => Promise<void> {
  return handleInsidersCommand;
}
...
export async function getFreshMessage(): Promise<string> {
  try {
    const { PrismaClient } = await import("@prisma/client");
    const prisma = new PrismaClient();
    ...
    // Find trades from wallets with < 5 total trades
    const freshTrades = await prisma.trade.findMany({
      where: {
        createdAt: { gte: oneDayAgo },
        usdValue: { gte: 5000 },
        wallet: {
      ...
      include: {
        market: { select: { question: true, eventSlug: true } },
        wallet: { select: { address: true, tradeCount: true, totalVolume: true, firstTradeAt: true } },
      },
    });
    ...
    if (freshTrades.length === 0) {
      return `🆕 *Fresh Wallet Activity*

━━━━━━━━━━━━━━━━━━━━━━

No significant trades from new wallets.

*What this means:*
New wallets with big positions can indicate
insider knowledge - they create new wallets
to avoid detection.

_Updated: ${new Date().toLocaleTimeString()}_`;
    }

    let message = `🆕 *Fresh Wallet Activity* (24h)\n\n`;
    message += `⚠️ *${freshTrades.length} trades from new wallets*\n`;
    message += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;
    ...
    for (const trade of freshTrades) {
      ...
      const side = trade.side === "BUY" ? "🟢 BUY" : "🔴 SELL";
      const size = trade.usdValue >= 1000 
        ? `$${(trade.usdValue / 1000).toFixed(1)}K`
      ...
      const tradeCount = wallet?.tradeCount || 1;
      ...
      message += `${side} *${size}*${isFirstTrade ? " 🎯 FIRST TRADE" : ""}\n`;
      message += `📈 ${marketDisplay}\n`;
      message += `👛 [${walletShort}](https://polygonscan.com/address/${walletAddr})\n`;
      message += `📊 Only ${tradeCount} trade${tradeCount > 1 ? "s" : ""} ever\n`;
      message += `🔗 [View Market](${polymarketUrl})\n\n`;
    }

    message += `━━━━━━━━━━━━━━━━━━━━━━\n`;
    message += `_🎯 = First ever trade - high signal_`;
    
    return message;
  } catch (error) {
    console.error("[TG-BOT] Error fetching fresh wallets:", error);
    return `🆕 *Fresh Wallet Activity*\n\n⚠️ Error fetching data. Try again later.`;
  }
}

export async function handleFreshCommand(ctx: Context): Promise<void> {
  await ctx.reply("🔄 Finding new wallet activity...");
  const message = await getFreshMessage();
  await ctx.reply(message, { 
    parse_mode: "Markdown",
    link_preview_options: { is_disabled: true },
  });
...
export function createFreshCommandHandler(): (ctx: Context) => Promise<void> {
  return handleFreshCommand;
}

// =============================================================================
// /markets - HOT MARKETS WITH REAL DATA
// =============================================================================
...
export async function getMarketsMessage(): Promise<string> {
  try {
    ...
    // Get markets with actual whale activity
    const markets = await prisma.market.findMany({
      where: { 
        active: true,
        volume: { gt: 0 },
...
Markets appear here when whale activity
is detected. Check back soon!
...
_Updated: ${new Date().toLocaleTimeString()}_`;
    ...
    for (let i = 0; i < markets.length; i++) {
      ...
      const volume24h = market.volume24h >= 1000 
        ? `$${(market.volume24h / 1000).toFixed(0)}K`
        : `$${market.volume24h.toFixed(0)}`;
      
      const tradeCount = (market as any)._count?.trades || 0;
      const whaleTrades = (market as any).trades?.length || 0;
      
      const eventSlug = market.eventSlug || market.slug;
      const polymarketUrl = `https://polymarket.com/event/${eventSlug}`;
      
      message += `${medal} *${question}*\n`;
      message += `   💰 Vol: ${volume} | 24h: ${volume24h}\n`;
      message += `   📊 ${tradeCount} trades | 🐋 ${whaleTrades} whale\n`;
      message += `   🔗 [Trade on Polymarket](${polymarketUrl})\n\n`;
    }

    message += `━━━━━━━━━━━━━━━━━━━━━━\n`;
    message += `_${new Date().toLocaleTimeString()} • /whales for trades_`;
...
export function getHelpMessage(): string {
  return `🐋 *PolyWhale Commands*

━━━━━━━━━━━━━━━━━━━━━━

*Intelligence:*
/whales - 🐋 Whale trades with follow signals
/insiders - 🕵️ Suspicious activity detection
/fresh - 🆕 New wallet big trades
/markets - 📊 Hot markets by volume
...
*Signal Legend:*
🟢 FOLLOW - High win rate, track record
🟡 MIXED - Inconsistent history
🔴 FADE - Poor track record
⚪ NEW - Not enough data

_Questions? @polywhale\\_support_`;
}
...
export async function registerUser(
  ctx: Context,
  service: TelegramSubscriberService = telegramSubscriberService
): Promise<RegistrationResult> {
  const chat = ctx.chat;
  const from = ctx.from;

  if (!chat) {
    return { success: false, isNewUser: false, wasReactivated: false, error: "No chat context" };
  }
  ...
  try {
    ...
    const existing = await service.findByChatId(BigInt(chat.id));
    
    if (existing) {
      if (!existing.isActive) {
        const reactivated = await service.reactivate(BigInt(chat.id));
        return { success: true, isNewUser: false, wasReactivated: true, subscriber: reactivated ?? undefined };
      }
      return { success: true, isNewUser: false, wasReactivated: false, subscriber: existing };
    }

    const subscriber = await service.create(input);
    return { success: true, isNewUser: true, wasReactivated: false, subscriber };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return { success: false, isNewUser: false, wasReactivated: false, error: message };
  }
}
...
export async function handleStartCommand(ctx: Context): Promise<void> {
  ...
  const displayName = getDisplayName(ctx);
  const message = getWelcomeMessage(result.isNewUser, displayName);
...
export function createStartCommandHandler(): (ctx: Context) => Promise<void> {
...
✅ Suspicious patterns
⬜ Price movements (coming soon)

*Thresholds:*
🐋 Whale: $10,000+
🆕 Fresh wallet: <5 trades
⚠️ Suspicion score: 50+

━━━━━━━━━━━━━━━━━━━━━━

_Customization coming soon!_
_Contact @polywhale\\_support for requests_`;
...
export function createAlertsCommandHandler(): (ctx: Context) => Promise<void> {
...
export async function getStatsMessage(): Promise<string> {
  try {
    const { PrismaClient } = await import("@prisma/client");
    const prisma = new PrismaClient();
    
    const [marketCount, tradeCount, walletCount, alertCount] = await Promise.all([
    ...
    const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
    const whaleTrades24h = await prisma.trade.count({
      where: { createdAt: { gte: oneDayAgo }, usdValue: { gte: 10000 } },
    });
    
    const totalVolume24h = await prisma.trade.aggregate({
    ...
    const vol24h = (totalVolume24h._sum.usdValue || 0);
    const volDisplay = vol24h >= 1000000 
...
*Database:*
📊 ${marketCount.toLocaleString()} active markets
💹 ${tradeCount.toLocaleString()} total trades
👛 ${walletCount.toLocaleString()} wallets tracked

*Last 24 Hours:*
🐋 ${whaleTrades24h} whale trades
💰 ${volDisplay} volume tracked
...
export function createStatsCommandHandler(): (ctx: Context) => Promise<void> {
